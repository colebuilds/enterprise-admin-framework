 ...
 * @name: BooleanApiResponse
 */
export type BooleanApiResponse = boolean;
...
export interface CountAndTimeZoneInfoRsp {
  /**
   * 国家Code
   */
  countryCode: string;
...
/**
 * @description: 创建调度任务 (Auth) 请求
 * @url: /api/SchedulerTask/Create
 * @name: CreateSchedulerTaskReq
 */
export interface CreateSchedulerTaskReq {
  /**
   * 任务名称
   */
  taskName: string;
   ...
   * 
   */
  requestMethod: SchedulerRequestMethod;
  /**
   * 开始时间
   */
  startTime: number;
...
export interface CurrencyInfoRsp {
  /**
   * 
   */
  currencyCode: string;
...
/**
 * @description: 删除调度任务 (Auth) 请求
 * @url: /api/SchedulerTask/Delete
 * @name: DeleteSchedulerTaskReq
 */
export interface DeleteSchedulerTaskReq {
  /**
   * 任务ID
   */
  id: number;
...
/**
 * @description: 删除调度任务的作业 (Auth) 请求
 * @url: /api/SchedulerTask/DeleteTaskJob
 * @name: DeleteTaskJobReq
 */
export interface DeleteTaskJobReq {
  /**
   * 任务ID
   */
  taskId: number;
...
/**
 * @description: 获取调度任务详情 (Auth) 请求
 * @url: /api/SchedulerTask/GetDetail
 * @name: GetSchedulerTaskDetailReq
 */
export interface GetSchedulerTaskDetailReq {
  /**
   * 任务ID
   */
  id: number;
...
/**
 * @description: 获取调度任务列表 (Auth) 请求
 * @url: /api/SchedulerTask/GetList
 * @name: GetSchedulerTaskListReq
 */
export interface GetSchedulerTaskListReq {
  /**
   * 任务名称
   */
  taskName?: string;
...
/**
 * @description: 获取调度任务日志列表 (Auth) 请求
 * @url: /api/SchedulerTask/GetLogs
 * @name: GetSchedulerTaskLogsReq
 */
export interface GetSchedulerTaskLogsReq {
  /**
   * 任务ID
   */
  id: number;
...
export interface GoogleVerifyInfo {
  /**
   * 是否绑定过谷歌验证码
   */
   ...
   * 谷歌秘钥
   */
  googleAccountSecretKey?: string;
}

export interface HealthCheckItemRsp {
  /**
   * 组件名称
   */
  name: string;
  /**
   * 检查类型：Read / Write / ReadWrite
   */
  checkType: string;
  /**
   * 状态：OK / Error
...
/**
 * @description: 健康检查：检测所有配置的外部服务是否可访问（读+写） 响应
 * @url: /api/Home/CheckHealth
 * @name: HealthCheckRsp
 */
export interface HealthCheckRsp {
  /**
   ...
   * 各组件检查结果
   */
  items: HealthCheckItemRsp[];
}

export type HealthCheckRspApiResponse = HealthCheckRsp;

/**
 * @description: 获取基础信息 响应
 * @url: /api/Login/HomeBasic
 * @name: HomeBasicInfoRsp
 */
export interface HomeBasicInfoRsp {
  /**
   * 集团Id
   */
  orgId: number;
...
export type HomeBasicInfoRspApiResponse = HomeBasicInfoRsp;

/**
 * @description: 获取当前系统用户的信息 (Auth) 响应
 * @url: /api/Home/GetSysUserInfo
 * @name: HomeSysUserInfoRsp
 */
export interface HomeSysUserInfoRsp {
  /**
   * 系统用户的登录名
   */
  userName: string;
   ...
   * 出款权限是否启用
   */
  withdraw_EnableState: boolean;
  /**
   * 审批权限是否启用
   */
  approval_UserState: boolean;
  /**
   * 谷歌账号
   */
   ...
   * 币种
   */
  currencyList: CurrencyInfoRsp[];
  /**
   * 时区
   */
  timeZoneList: CountAndTimeZoneInfoRsp[];
  /**
   * 语言
   */
  languageList: CodeNameRsp[];
  /**
   * 是否是超级管理员，包含导出报表手机号码
   */
  isSuperAuthUser: boolean;
  /**
   * 集团列表（总控返回所有，集团管理员返回自己所属集团）
   */
  orgList: IdNameRsp[];
  /**
   * 商户列表（总控返回所有，集团管理员返回本集团下的商户）
   */
  tenantList: TenantInfoRsp[];
  /**
   * 充值权限商户列表
   */
  approvalTenantList: TenantInfoRsp[];
  /**
   * V3存储桶图片域名（新上传的图片路径以 lotto/ 开头，用此域名拼接）
   */
  v3OssImageUrl: string;
}

export type HomeSysUserInfoRspApiResponse = HomeSysUserInfoRsp;
...
/**
 * @description: 1 - Second 2 - Minute 3 - Hour 4 - Day
 */
export type IntervalTimeType = 1 | 2 | 3 | 4;
...
1. 如果系统和用户设置都开启了谷歌验证，
    ...
    1.2 用户通过谷歌验证码检验了，请求参数中没有 vCode，返回错误信息： {    "data": null,    "code": 1,    "msg": "vCode isn't correct",    "msgCode": 41} 请求
 * @url: /api/Login/Login
 * @name: LoginReq
 */
...
1. 如果系统和用户设置都开启了谷歌验证，
    1.1 用户没有谷歌验证码核验，就需要 vCode 参数进行绑定，否则返回信息中包含: "googleVerifyInfo": {"googleImageUrl": "二维码图片地址","googleVerify": false} 
    1.2 用户通过谷歌验证码检验了，请求参数中没有 vCode，返回错误信息： {    "data": null,    "code": 1,    "msg": "vCode isn't correct",    "msgCode": 41} 响应
 * @url: /api/Login/Login
 * @name: LoginRsp
 */
export interface LoginRsp {
   ...
   * 允许密码错误最大次数
   */
  passwordErrorMaxNum: number;
  /**
   * WebSocket连接地址（含Token参数）
后台登录成功后，前端可以使用该地址建立WebSocket连接，以接收后台推送的消息
需要订阅：
    1. /AdminApi/{OrgId}/all/   - 接收集团范围内的广播消息
    2. /AdminApi/{OrgId}/UserId/{UserId}/ - 接收发送给当前登录账户的消息
   */
  webSocketUrl: string;
  /**
   * 需要订阅的频道列表
前端在WebSocket连接成功后，按顺序订阅这些频道
   */
  channels: WebSocketChannelInfo[];
  /**
   * 
   */
  googleVerifyInfo: GoogleVerifyInfo;
}

export type LoginRspApiResponse = LoginRsp;
...
/**
 * @description: 暂停调度任务 (Auth) 请求
 * @url: /api/SchedulerTask/Pause
 * @name: PauseSchedulerTaskReq
 */
export interface PauseSchedulerTaskReq {
  /**
   * 任务ID
   */
  id: number;
...
export interface RechargeLevelRsp {
  /**
   * 用户等级
   */
  userLevel: number;
...
/**
 * @description: 为多个集团注册所有调度任务 (Auth) 请求
 * @url: /api/SchedulerTask/RegisterTasksForTenants
 * @name: RegisterTasksForTenantsReq
 */
export interface RegisterTasksForTenantsReq {
  /**
   * 集团ID列表
   */
  orgIds: number[];
...
/**
 * @description: 恢复调度任务 (Auth) 请求
 * @url: /api/SchedulerTask/Resume
 * @name: ResumeSchedulerTaskReq
 */
export interface ResumeSchedulerTaskReq {
  /**
   * 任务ID
   */
  id: number;
...
/**
 * @description: 立即执行调度任务 (Auth) 请求
 * @url: /api/SchedulerTask/RunNow
 * @name: RunNowSchedulerTaskReq
 */
export interface RunNowSchedulerTaskReq {
  /**
   * 任务ID
   */
  taskId: number;
...
/**
 * @description: 1 - Get 2 - Post 3 - Put 4 - Delete
 */
export type SchedulerRequestMethod = 1 | 2 | 3 | 4;

export interface SchedulerTaskDetailListRsp {
  /**
   * 任务名称
   */
  taskName: string;
...
/**
 * @description: 获取调度任务详情 (Auth) 响应
 * @url: /api/SchedulerTask/GetDetail
 * @name: SchedulerTaskDetailListRspListPageBaseResponse
 */
export interface SchedulerTaskDetailListRspListPageBaseResponse {
  /**
   * 获取 返回数据
   */
  list: SchedulerTaskDetailListRsp[];
  /**
   * 当前页
   */
  pageNo: number;
...
export type SchedulerTaskDetailListRspListPageBaseResponseApiResponse = SchedulerTaskDetailListRspListPageBaseResponse;
...
/**
 * @description: 获取调度任务列表 (Auth) 响应
 * @url: /api/SchedulerTask/GetList
 * @name: SchedulerTaskDtoListApiResponse
 */
export type SchedulerTaskDtoListApiResponse = SchedulerTaskDto[];

export interface SchedulerTaskLogRsp {
  /**
   * 
   */
  primaryKey: string;
...
/**
 * @description: 获取调度任务日志列表 (Auth) 响应
 * @url: /api/SchedulerTask/GetLogs
 * @name: SchedulerTaskLogRspListPageBaseResponse
 */
export interface SchedulerTaskLogRspListPageBaseResponse {
  /**
   * 获取 返回数据
   */
  list: SchedulerTaskLogRsp[];
  /**
   * 当前页
   */
  pageNo: number;
...
export type SchedulerTaskLogRspListPageBaseResponseApiResponse = SchedulerTaskLogRspListPageBaseResponse;

/**
 * @description: 1 - Url
 */
export type SchedulerTaskType = 1;

/**
 * @description: 1 - Cron 2 - Simple
 */
export type SchedulerTriggerType = 1 | 2;
...
/**
 * @description: 用来自测的： 返回一个字符串，看看 k8s 是否发布成功 响应
 * @url: /api/Home/GetVersion
 * @name: StringApiResponse
 */
export type StringApiResponse = string;

export interface TenantInfoRsp {
  /**
   * 商户ID
   */
  id: number;
  ...
  /**
   * V1存储桶图片域名（用于历史图片路径，不以 lotto/ 开头的路径用此域名拼接）
   */
  v1OssImageUrl: string;
  /**
   * 商户支持的充值大类列表
   */
  rechargeCategoryList: IdNameRsp[];
  /**
   * V1充值等级列表
   */
  rechargeLevelList: RechargeLevelRsp[];
}

/**
 * @description: 0 - 已关站 1 - 运营中 2 - 维护中
 */
export type TenantStateEnum = 0 | 1 | 2;

/**
 * @description: 0 - Normal 1 - Paused 2 - Complete 3 - Error 4 - Blocked 5 - None
 */
export type TriggerState = 0 | 1 | 2 | 3 | 4 | 5;

/**
 * @description: 更新调度任务 (Auth) 请求
 * @url: /api/SchedulerTask/Update
 * @name: UpdateSchedulerTaskReq
 */
export interface UpdateSchedulerTaskReq {
  /**
   * 任务ID
   */
  id: number;
...
/**
 * @description: 启用调度任务 (Auth) 请求
 * @url: /api/SchedulerTask/Enable
 * @name: UpdateTaskStateReq
 */
export interface UpdateTaskStateReq {
  /**
   * 任务ID
   */
  id: number;
...
export interface WebSocketChannelInfo {
  /**
   * 频道路径(如: /AdminApi/1/all, /AdminApi/1/userid/123)
   */
  channel?: string;
  /**
   * 频道类型(all=广播, target=定向)
   */
  channelType?: string;
  /**
   * 频道描述(供前端调试/日志使用)
...
/**
 * @description: 启动探针 确认程序是否正常启动 请求
 * @url: /api/Probe/GetStartupProbe
 * @name: GetStartupProbeQueryDto
 */
export interface GetStartupProbeQueryDto {
  /**
   * 密钥
   */
  SecretKey?: string;
...
/**
 * @description: 就绪探针 确认程序是否就绪 请求
 * @url: /api/Probe/GetReadinessProbe
 * @name: GetReadinessProbeQueryDto
 */
export interface GetReadinessProbeQueryDto {
  /**
   * 密钥
   */
  SecretKey?: string;
...
/**
 * @description: 获取静态缓存数据 请求
 * @url: /api/StaticCache/GetStaticCache
 * @name: GetStaticCacheQueryDto
 */
export interface GetStaticCacheQueryDto {
  /**
   * 
   */
  tableName?: string;
