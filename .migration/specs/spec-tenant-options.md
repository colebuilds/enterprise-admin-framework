...
> 源文件：`feat1-ar_platform_admin/src/hooks/useTenantOptions.ts`（955行）目标：3 个文件，约 500 行，公共接口不变，调用方零改动。
...
**为什么需要 `#/utils/tenant.ts`：** store 需要 `formatTenantLabel` 和 `TenantOption` 类型，hook 需要 store。如果 `formatTenantLabel` 放在 hook 里，就形成 store→hook→store 循环依赖。提取到 utils 彻底断开循环。
...
> 非管理员用户登录后调用 `load()` 时，store 的非管理员短路路径会立即将 `loaded` 设回 `true`，因此 `reset()` 在 logout 时安全调用，不影响下一个普通用户的正常使用。
...
setup-syntax store 返回的 `ref`/`computed` 在外部通过 `store.xxx` 访问时**自动解包**，返回的是值本身（`T`），不是 `Ref<T>`。
...
> - `getTenantTimeZone()` / `getTenantTimeZoneLabel()` 读 `store.tenantTimeZoneMap`（non-admin），超管对 admin API 独有的商户时区查询同样返回 fallback
...
以下方法逻辑简单，实现时对照原文件 line 730-870 直接搬入，把 `state.tenantTimeZoneMap.value` → `store.tenantTimeZoneMap`，把 `state.tenantMap.value` → `store.tenantMap`：
...
return {
  ...
  tenantCascaderOptions: computed(() =>
    isSuper.value ? store.tenantCascaderOptions : [],
  ),
...
```ts
const { tenantOptions, getTenantName, formatTenantDateTime } =
  useTenantOptions();
```
