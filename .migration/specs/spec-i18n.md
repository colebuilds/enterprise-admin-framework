...
export type Namespace =
  | 'common' | 'finance' | 'member' | 'tenant' | 'system'
  | 'operations' | 'game' | 'report' | 'analytics' | 'dashboard' | 'account'
  | 'hubConfig' | 'productNav' | 'erde' | 'agent';
...
// namespace 懒加载模块映射
const moduleLoaders: Record<SupportedLocale, Record<Namespace, () => Promise<any>>> = {
  en: {
...
export async function setupI18n(app: App, locale: SupportedLocale = DEFAULT_LOCALE) {
  i18n = createI18n({
    legacy: false,
    locale,
    fallbackLocale: 'en',
  ...
  await Promise.all(
    toLoad.map(async (ns) => {
      try {
        ...
        i18n.global.mergeLocaleMessage(currentLocale, { [ns]: mod.default ?? mod });
...
export function collectRouteNamespaces(to: RouteLocationNormalized): Set<Namespace> {
  const namespaces = new Set<Namespace>();

  for (const record of to.matched) {
    const declared = record.meta?.i18nNamespace as Namespace[] | Namespace | undefined;
...
import { zhCN, enUS, viVN, idID, dateZhCN, dateEnUS, dateViVN, dateIdID } from 'naive-ui';
