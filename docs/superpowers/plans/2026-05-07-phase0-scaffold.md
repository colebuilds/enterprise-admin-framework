...
```ts
  // 业务插件
  const { setupNaive, setupNaiveDiscreteApi, setupDirectives } = await import('#/plugins');
  setupNaive(app);
  setupDirectives(app);
...
const tokenLocale = computed(
  () => NAIVE_LOCALE_MAP[preferences.app.locale as keyof typeof NAIVE_LOCALE_MAP]?.locale ?? enUS,
);
const tokenDateLocale = computed(
  () => NAIVE_LOCALE_MAP[preferences.app.locale as keyof typeof NAIVE_LOCALE_MAP]?.dateLocale ?? dateEnUS,
);
